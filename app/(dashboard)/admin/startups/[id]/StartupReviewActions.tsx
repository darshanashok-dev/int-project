...
export default function StartupReviewActions({
  ...
  const statuses = [
    { id: 'active',     label: 'Approve & Activate',   icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50 hover:bg-emerald-100', border: 'border-emerald-200' },
    { id: 'pending',    label: 'Mark Pending Review',   icon: Clock,        color: 'text-amber-600',   bg: 'bg-amber-50 hover:bg-amber-100',     border: 'border-amber-200'   },
    { id: 'waitlisted', label: 'Move to Waitlist',      icon: Briefcase,    color: 'text-purple-600',  bg: 'bg-purple-50 hover:bg-purple-100',   border: 'border-purple-200'  },
    { id: 'rejected',   label: 'Reject Venture',        icon: XCircle,      color: 'text-rose-600',    bg: 'bg-rose-50 hover:bg-rose-100',       border: 'border-rose-200'    },
  ]
  ...
  return (
    <div className="bg-[#202124] rounded-3xl p-6 shadow-xl space-y-6 text-white relative overflow-hidden">
      <div className="absolute -bottom-20 -right-20 w-40 h-40 bg-blue-500/20 blur-[80px] rounded-full" />
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-2 relative z-10">
      ...
      <div className="pt-6 border-t border-white/10 relative z-10 space-y-4">
        ...
        <button
          ...
          disabled={isProcessing}
          className="w-full flex items-center justify-center gap-2 py-3 bg-white text-black rounded-xl font-bold text-sm hover:bg-gray-200 transition-colors disabled:opacity-50"
        >
          {isProcessing ? (
